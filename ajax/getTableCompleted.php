...
include "../connection.php";
                    ...
                    $id_user = $find["id"];
                    $output = "";
                    $list = "SELECT orders.id,orders.alamat,orders.nama,orders.jenis,orders.imgBefore,orders.imgAfter,orders.status, user_data.username, orders.tglOrder, order_details.longHours,order_details.cost FROM `orders` JOIN user_data ON user_data.id = orders.idDesigner JOIN order_details ON orders.id = order_details.idOrder WHERE idDesigner = '$id_user' AND (status = 7 OR status = 9)";
                    ...
                    while ($result = mysqli_fetch_assoc($action)){
                        ...
                        if($result['status'] == 7){
                            $output.=
                            '
                            <td class="text-center" id="action-'.$result['id'].'">Menunggu admin transfer.</td>
                            </tr> 
