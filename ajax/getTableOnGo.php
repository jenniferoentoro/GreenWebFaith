...
include "../connection.php";
                    ...
                    $id_user = $find["id"];
                    $output = "";
                    $list = "SELECT orders.id,orders.alamat,orders.nama,orders.jenis,orders.imgBefore,orders.imgAfter,orders.status, user_data.username, orders.tglOrder, order_details.longHours,order_details.cost, noresides.noresi AS noresides ,noresicus.noresi AS noresicus FROM `orders` JOIN user_data ON user_data.id = orders.idDesigner JOIN order_details ON orders.id = order_details.idOrder LEFT JOIN noresicus ON noresicus.idOrder = orders.id LEFT JOIN noresides ON noresides.idOrder = orders.id WHERE idUser= '$id_user' AND (status = 2 OR status = 3 OR status = 4 OR status = 5 OR status = 6 OR status = 8)";
