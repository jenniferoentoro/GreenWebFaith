...
/*	echo "<pre>";
	...
	if(mysqli_num_rows($getID) == 1){
		 if($telp != '' &&$id != ''  && !empty($image)){
				# Text input
				# Check extension
				if (in_array(strtolower($extension), $valid_extensions)){
					# Moving the File 1
						$file_address = $_SERVER["DOCUMENT_ROOT"].'/GreenWebFaith/transferproof/'.$newName;
						$file_foto = 'transferproof/'.$newName;
					...
					if ($moved){
						...
						$sql_text_1 = "INSERT INTO `orderacc`(`id`, `noTelp`, `transfer_proof`, `idOrder`) VALUES (null,'$telp','$file_foto','$id')";
						$inserted_1 = mysqli_query($con, $sql_text_1);
                        $sql_text_2 = "UPDATE `orders` SET `status`=2 WHERE id='$id'";
